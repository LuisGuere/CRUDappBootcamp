...
const muestraRegistros = () => {
  const registrosLocales = JSON.parse(localStorage.getItem("usuarios")) || [];
    ...
    registrosLocales.forEach((registroLocal) => {
      ...
      <td class="row">
      <div class="col-sm-4">
      <button type="button" id="boton4" class="btn btn-dark" onclick="iniciarEditarRegistro('${registroLocal.id}')">Editar</button>
      </div>            
      <div class="col-sm-4">
      <button type="button" id="boton3" class="btn btn-dark" onclick="eliminarRegistro('${registroLocal.id}')">Eliminar</button>
      </div>
      </td>
      </tr>`;
